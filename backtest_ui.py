...
def render_backtest_dashboard():
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Industry P/E": st.column_config.NumberColumn("Ind P/E", format="%.1f"),
            "Market Cap (₹ Cr)": st.column_config.NumberColumn("M.Cap (₹ Cr)", format="₹%,.0f Cr"),
            "YoY Rev Growth %": st.column_config.NumberColumn("YoY Rev %", format="%+.1f%%"),
            "YoY Profit Growth %": st.column_config.NumberColumn("YoY PAT %", format="%+.1f%%"),
            "Volume Growth 1W %": st.column_config.NumberColumn("1W Vol %", format="%+.1f%%"),
            "Volume Growth 1M %": st.column_config.NumberColumn("1M Vol %", format="%+.1f%%"),
            "ROE %": st.column_config.NumberColumn("ROE %", format="%.1f%%"),
            "Debt to Equity": st.column_config.NumberColumn("D/E", format="%.2f"),
            "Operating Margin %": st.column_config.NumberColumn("OPM %", format="%.1f%%"),
        },
        use_container_width=True,
        hide_index=True
    )
